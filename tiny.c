...
void doit(int fd) 
{
    ...
    if (strcasecmp(method, "GET") == 0){
        ...
        if (is_file_valid(filename, &sbuf, fd)) {
            if (is_static) { /* Serve static content */          
                ...
                serve_static(fd, filename, sbuf.st_size, 0);        //line:netp:doit:servestatic
            }
                ...
                serve_dynamic(fd, filename, cgiargs, 0);            //line:netp:doit:servedynamic
            }
        }
        ...
        if (is_file_valid(filename, &sbuf, fd)) {
            if (is_static) { /* Serve static content */          
                ...
                serve_static(fd, filename, sbuf.st_size, 1);        //line:netp:doit:servestatic
            }
            else { /* Serve dynamic content */
                if (!(S_ISREG(sbuf.st_mode)) || !(S_IXUSR & sbuf.st_mode)) { //line:netp:doit:executable
                    clienterror(fd, filename, "403", "Forbidden",
                ...
                serve_dynamic(fd, filename, cgiargs, 1);            //line:netp:doit:servedynamic
            }
        }
    } else {
